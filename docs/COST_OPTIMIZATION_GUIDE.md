...
|------------------|-----|------|-------------------|---------|
| **g4dn.2xlarge** | T4 | 標準 | ~$1.052 | ⭐ **学習用推奨**（AMI最小要件） |
| g4dn.4xlarge | T4 | 高 | ~$1.60 | より大きなシミュレーション |
| g5.xlarge | A10G | 高 | ~$1.006 | 高性能が必要な場合（A10G GPU） |
| **g6e.xlarge** | L40S | 最高 | ~$1.20+ | 最新機能・最高性能が必要な場合 |

**推奨**: 個人学習では `g4dn.2xlarge` が最適です。

- Isaac Sim AMI が要求する最小メモリ(32GB)を満たす
- コストと性能のバランスが良い
- **注意**: `g4dn.xlarge` はサポート対象外です
...
| 東京（ap-northeast-1） | ~$1.05 | 低（日本） |
| 米国東部（us-east-1） | ~$0.75 | 中 |
| 米国西部（us-west-2） | ~$0.75 | 中 |
...
```json
{
  ...
  "SpotInstanceMaxPrice": "0.30",
  "AutoShutdownEnabled": "true"
}
```

**月額コスト**: 約 $40-60（1日2-3時間使用の場合）
